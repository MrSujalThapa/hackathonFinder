import { z } from "zod";
import { decodeOwnerPasswordHashB64 } from "@/lib/auth/password";
...
export const serverEnvSchema = z
  .object({
    ...
    APP_OWNER_PASSWORD_HASH: optionalString,
    APP_OWNER_PASSWORD_HASH_B64: optionalString,
...
export function validateProductionConfig(env: ServerEnv = getServerEnv()): string[] {
  ...
  if (!hasOwnerAuthConfig(env)) {
    issues.push(
      "Owner auth is incomplete: set APP_OWNER_PASSWORD_HASH_B64 (preferred) or APP_OWNER_PASSWORD_HASH, plus APP_SESSION_SECRET.",
    );
  }
  if ((env.APP_SESSION_SECRET?.length ?? 0) < 32) {
    issues.push("APP_SESSION_SECRET must be at least 32 characters.");
  }
  if (env.APP_OWNER_PASSWORD_HASH_B64) {
    if (!decodeOwnerPasswordHashB64(env.APP_OWNER_PASSWORD_HASH_B64)) {
      issues.push("APP_OWNER_PASSWORD_HASH_B64 is malformed. Re-run npm run hash:password.");
    }
  } else if (
    env.APP_OWNER_PASSWORD_HASH &&
    !env.APP_OWNER_PASSWORD_HASH.replace(/^["']|["']$/g, "").startsWith("scrypt$")
  ) {
    issues.push("APP_OWNER_PASSWORD_HASH must be generated by npm run hash:password.");
  }
