...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (parsed.data.action === "preview") return ok({ application: await previewSubmission(current.id), message: "The real form was filled and reconciled in the durable browser profile. No submission was attempted." });
    if (parsed.data.action === "submit_now") { const application = await submitAuthorizedDraft(current.id); return ok({ application, message: application.status === "submitted" ? "Fixture application submitted and snapshot saved." : "Submission blocked: the real form did not match this draft." }); }
