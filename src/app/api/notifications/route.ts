...
import { countUnreadNotifications, listNotifications, setNotificationRead } from "@/server/notifications/service";
export async function GET(request: Request) { try { if (new URL(request.url).searchParams.get("summary") === "true") return ok({ unreadCount: await countUnreadNotifications() }); return ok({ notifications: await listNotifications() }); } catch { return fail("INTERNAL_ERROR", "Could not load notifications.", 500); } }
