...
export async function PUT(request: Request) {
  ...
  try { const existing = parsed.data.id ? (await listAssetBank()).find((asset) => asset.id === id) : null; const uploaded = await uploadAssetFile({ assetId: id, filename: file.name, file }); const asset = await upsertAssetBank({ id, label: parsed.data.label, kind: "file", assetType: parsed.data.assetType, value: uploaded.value, filename: uploaded.filename, notes: JSON.stringify({ size: file.size, mimeType: file.type || "application/octet-stream", uploadedAt: new Date().toISOString() }), isDefault: parsed.data.isDefault === "true" }); if (existing && existing.value !== uploaded.value) await deleteStoredAsset(existing.value); return ok({ asset }, { status: existing ? 200 : 201 }); } catch (error) { return fail("INTERNAL_ERROR", error instanceof Error ? error.message : "Could not upload asset.", 500); }
}
