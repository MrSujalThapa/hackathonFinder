...
import type { ApplicationDraft, ApplicationQuestion, ApplicationStatus, AssetBankEntry, Profile, QuestionBankEntry, QuestionBankFolder } from "@/core/applications/types";

const OWNER_ID = "00000000-0000-0000-0000-000000000001";
const DEFAULT_QUESTION_FOLDERS = ["Career", "Personal", "Fun", "Hackathons", "Startup", "Technical"];
const FOLDER_COLORS = ["blue", "teal", "purple", "amber", "rose", "indigo"];
...
export async function listQuestionBank(): Promise<QuestionBankEntry[]> {
  const db = createServiceSupabaseClient(); const { data, error } = await db.from("question_bank").select("*").eq("user_id", OWNER_ID).order("updated_at", { ascending: false }); if (error) throw new Error(`Could not load Question Bank: ${error.message}`); return (data ?? []).map((row) => ({ id: row.id, folderId: row.folder_id, canonicalQuestion: row.canonical_question, answer: row.answer, aliases: row.aliases, tags: row.tags, updatedAt: row.updated_at }));
}
export async function upsertQuestionBank(entry: Omit<QuestionBankEntry, "updatedAt">): Promise<QuestionBankEntry> {
  const db = createServiceSupabaseClient(); const { data, error } = await db.from("question_bank").upsert({ id: entry.id, user_id: OWNER_ID, folder_id: entry.folderId ?? null, canonical_question: entry.canonicalQuestion, answer: entry.answer, aliases: entry.aliases, tags: entry.tags }).select("*").single(); if (error) throw new Error(`Could not save Question Bank entry: ${error.message}`); return { id: data.id, folderId: data.folder_id, canonicalQuestion: data.canonical_question, answer: data.answer, aliases: data.aliases, tags: data.tags, updatedAt: data.updated_at };
}
...
function normalizeFolder(value: string): string { return value.trim().replace(/\s+/g, " ").slice(0, 80); }
export async function listQuestionBankFolders(): Promise<QuestionBankFolder[]> { const db = createServiceSupabaseClient(); const { data, error } = await db.from("question_bank_folders").select("*").eq("user_id", OWNER_ID).order("name"); if (error) throw new Error(`Could not load Question Bank folders: ${error.message}`); if (data?.length) return data.map((row) => ({ id: row.id, name: row.name, color: row.color, createdAt: row.created_at })); const { error: seedError } = await db.from("question_bank_folders").insert(DEFAULT_QUESTION_FOLDERS.map((name, index) => ({ user_id: OWNER_ID, name, color: FOLDER_COLORS[index] }))); if (seedError) throw new Error(`Could not seed Question Bank folders: ${seedError.message}`); return listQuestionBankFolders(); }
export async function createQuestionBankFolder(name: string, color = "blue"): Promise<QuestionBankFolder[]> { const folder = normalizeFolder(name); if (!folder) throw new Error("Folder name is required."); const db = createServiceSupabaseClient(); const { error } = await db.from("question_bank_folders").insert({ user_id: OWNER_ID, name: folder, color }); if (error) throw new Error(`Could not create folder: ${error.message}`); return listQuestionBankFolders(); }
export async function renameQuestionBankFolder(id: string, name: string): Promise<QuestionBankFolder[]> { const folder = normalizeFolder(name); if (!folder) throw new Error("Folder name is required."); const db = createServiceSupabaseClient(); const { error } = await db.from("question_bank_folders").update({ name: folder }).eq("id", id).eq("user_id", OWNER_ID); if (error) throw new Error(`Could not rename folder: ${error.message}`); return listQuestionBankFolders(); }
export async function deleteQuestionBankFolder(id: string): Promise<QuestionBankFolder[]> { const db = createServiceSupabaseClient(); const { error } = await db.from("question_bank_folders").delete().eq("id", id).eq("user_id", OWNER_ID); if (error) throw new Error(`Could not delete folder: ${error.message}`); return listQuestionBankFolders(); }
