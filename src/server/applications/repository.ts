...
const QUESTION_FOLDERS_KEY = "_hackfinderQuestionFolders";
...
export async function listQuestionBankFolders(): Promise<string[]> { const profile = await getProfile(); try { const saved = JSON.parse(profile[QUESTION_FOLDERS_KEY] ?? "[]"); const custom = Array.isArray(saved) ? saved.filter((value): value is string => typeof value === "string").map(normalizeFolder).filter(Boolean) : []; return [...new Set([...DEFAULT_QUESTION_FOLDERS, ...custom])].sort(); } catch { return DEFAULT_QUESTION_FOLDERS; } }
async function saveQuestionBankFolders(folders: string[]): Promise<void> { const profile = await getProfile(); profile[QUESTION_FOLDERS_KEY] = JSON.stringify([...new Set(folders.map(normalizeFolder).filter(Boolean))]); await saveProfile(profile); }
export async function createQuestionBankFolder(name: string): Promise<string[]> { const folder = normalizeFolder(name); if (!folder) throw new Error("Folder name is required."); const folders = await listQuestionBankFolders(); await saveQuestionBankFolders([...folders, folder]); return [...new Set([...folders, folder])].sort(); }
export async function renameQuestionBankFolder(from: string, to: string): Promise<string[]> { const oldFolder = normalizeFolder(from); const newFolder = normalizeFolder(to); if (!oldFolder || !newFolder) throw new Error("Folder names are required."); const entries = await listQuestionBank(); await Promise.all(entries.filter((entry) => entry.tags[0] === oldFolder).map((entry) => upsertQuestionBank({ ...entry, tags: [newFolder, ...entry.tags.slice(1)] }))); const folders = await listQuestionBankFolders(); await saveQuestionBankFolders(folders.filter((folder) => folder !== oldFolder).concat(newFolder)); return listQuestionBankFolders(); }
export async function deleteQuestionBankFolder(name: string): Promise<string[]> { const folder = normalizeFolder(name); const entries = await listQuestionBank(); await Promise.all(entries.filter((entry) => entry.tags[0] === folder).map((entry) => upsertQuestionBank({ ...entry, tags: entry.tags.slice(1) }))); const folders = await listQuestionBankFolders(); await saveQuestionBankFolders(folders.filter((item) => item !== folder)); return listQuestionBankFolders(); }
