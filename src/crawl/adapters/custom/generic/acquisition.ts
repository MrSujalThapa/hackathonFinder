...
import { enumerateCandidateActionsFromHtml, verifyActionStateProgression } from "@/crawl/adapters/custom/generic/browserActions";
import type {
  AcquiredArtifact,
...
import { byteLength, isPlainRecord, isSafePublicOrigin } from "@/crawl/adapters/custom/generic/valueUtils";
...
export function actionStateFingerprint(html: string, baseUrl: string): string {
...
export function visibleIdentityKeys(html: string, baseUrl: string): Set<string> {
...
export function detectBlockedStateFromHtml(html: string): string | undefined {
...
export async function waitForDomStability(page: PageLike, samples: BrowserObservationSample[] = []): Promise<void> {
  let previous = "";
  let stable = 0;
...
export async function scrollNestedContainers(page: PageLike): Promise<number> {
...
export async function scrollProbeState(page: PageLike): Promise<ScrollProbeState> {
  return page.evaluate(() => {
    const elements = [...document.querySelectorAll("body *")] as HTMLElement[];
    const visible = elements.filter((element) => {
      const box = element.getBoundingClientRect();
...
export async function scrollPrimaryContainer(page: PageLike): Promise<ScrollProbeState> {
  return page.evaluate(() => {
    const elements = [...document.querySelectorAll("body *")] as HTMLElement[];
    const scrollers = elements
      .filter((element) => {
...
export async function executeCandidateAction(page: Page, action: CandidateAction): Promise<void> {
