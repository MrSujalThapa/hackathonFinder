...
export function ApplicationsList() {
  ...
  return <ul className="space-y-3" aria-label="Applications">{applications.map((application) => <li key={application.id} className="hf-sheet-frame p-4"><Link className="hf-focus block" href={`/drafts/${application.id}`}><div className="flex items-center justify-between gap-3"><span className="font-medium">Application draft</span><span className="font-mono text-xs text-muted">{application.status.replaceAll("_", " ")}</span></div><p className="mt-2 text-sm text-muted">{application.questions.filter((question) => question.answer).length}/{application.questions.length} fields answered{application.currentPage ? ` · Page ${application.currentPage}${application.totalPages ? `/${application.totalPages}` : ""}` : ""}</p></Link></li>)}</ul>;
}
