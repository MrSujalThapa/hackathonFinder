...
export function NotificationBell() {
  ...
  useEffect(() => { const controller = new AbortController(); fetch("/api/notifications?summary=true", { signal: controller.signal }).then((response) => response.json()).then((body: { data?: { unreadCount?: number } }) => setCount(body.data?.unreadCount ?? 0)).catch(() => setCount(0)); return () => controller.abort(); }, []);
