...
export function NotificationBell() {
  ...
  useEffect(() => { const controller = new AbortController(); fetch("/api/notifications", { signal: controller.signal }).then((response) => response.json()).then((body: { data?: { notifications?: Array<{ isRead?: boolean }> } }) => setCount(body.data?.notifications?.filter((item) => !item.isRead).length ?? 0)).catch(() => setCount(0)); return () => controller.abort(); }, []);
