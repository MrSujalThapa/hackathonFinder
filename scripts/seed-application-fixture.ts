...
async function main(): Promise<void> {
  ...
  const db = createServiceSupabaseClient(); const fingerprint = "pass1-gateway-fixture-hackmit";
  ...
  if (!candidateId) { const { data, error } = await db.from("candidates").insert({ name: "HackMIT Gateway Fixture", source: "pass1_fixture", fingerprint, score: 0, apply_url: "https://fixture.invalid/hackmit", themes: [], why_match: [], red_flags: [] }).select("id").single(); if (error) throw error; candidateId = data.id; }
  ...
  console.log(JSON.stringify({ candidateId, applicationId: application.id }));
}
